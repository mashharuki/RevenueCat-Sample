...
export async function fetchShowtimes(movieId: string): Promise<Result<Showtime[]>> {
  await delay();
  const showtimes = SHOWTIMES.filter((showtime) => showtime.movieId === movieId);
...
export async function fetchShowtimeById(showtimeId: string): Promise<Result<Showtime>> {
  await delay();
