...
export async function purchaseTicket(input: PurchaseInput): Promise<Result<Ticket>> {
  await delay();

  if (MockConfig.alwaysFailPurchase) {
    return err("SEATS_TAKEN", "One or more selected seats were just booked by someone else.");
  }
  ...
  const seats = input.seatIds.map((seatId) => parseSeatId(seatId, seatMapResult.data.vipRows));
