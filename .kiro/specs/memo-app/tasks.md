...
- [ ] 8.4 E2Eフロー検証
  ...
  - _Depends: 7.1, 7.2_
  - _Blocked: 3フロー中2つ(上限到達→ペイウォール→購入成功→再試行成功、購入の復元によるペイウォール解除)は`FakeLimitThenSuccessClient`を`MemoApp.swift`に一時注入し、実際のRevenueCat Test Storeに対してシミュレータ上で確認済み(検証後`git checkout`で復元、コード変更なし)。残る「サインイン→メモ一覧到達」はGoogle実アカウントでの認証が必須で、パスワード等の認証情報入力はエージェントの安全ポリシー上代行できないため、ユーザー自身の環境での手動確認に委ねることで合意した。ユーザー環境でシミュレータのキーボード非表示・CoreSimulator(launchd_sim)クラッシュが発生し、Mac再起動を経て復旧したが、セッション内でのサインイン試行までは至らなかった。_
