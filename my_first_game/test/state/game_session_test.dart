...
void main() {
  ...
  test(
    'should switch to paused on pauseGame and back to playing on resumeGame',
    () {
  ...
  test(
    'should record a new high score and best run on endGame when beaten',
    () {
      final session = GameSession();
  ...
  test(
    'should include the player as YOU in leaderboardRows after a best run',
    () {
      final session = GameSession();
  ...
  test(
    'should increment runId on every startGame call so a fresh run can be identified',
    () {
      final session = GameSession();
  ...
  test(
    'should increment runId on restartGame since it delegates to startGame',
    () {
      final session = GameSession()..startGame();
  ...
  test(
    'should show the paywall with a banner and return to the requested screen on close',
    () {
      final session = GameSession();
  ...
  test(
    'should route reachWaveLimit to the paywall and preserve the current run as lastResult',
    () {
      final session = GameSession()
        ..startGame()
  ...
  test(
    'should route showWeeklyChallenge to the paywall when not subscribed',
    () {
      final session = GameSession();
  ...
  test(
    'should spend one token, refill lives, and resume playing on continueGame',
    () {
      final session = GameSession()
