...
void main() {
  ...
  testWithGame<InvaderGame>('should mount a player after onLoad', () => game, (
    game,
  ) async {
    expect(game.player, isNotNull);
  });

  testWithGame<InvaderGame>(
    'should spawn 15 enemies for wave 1',
    () => InvaderGame(session: GameSession()),
    (game) async {
      game.spawnWave(1);
      await game.ready();
  ...
  testWithGame<InvaderGame>(
    'should reduce live enemy count and add score on registerHit',
    () => InvaderGame(session: GameSession()),
    (game) async {
      game.spawnWave(1);
      await game.ready();
  ...
  testWithGame<InvaderGame>(
    'should advance the enemy formation horizontally on update',
    () => InvaderGame(session: GameSession()),
    (game) async {
      game.spawnWave(1);
      await game.ready();
  ...
  testWithGame<InvaderGame>(
    'should not fire an enemy bullet before the shot interval elapses',
    () => InvaderGame(session: GameSession()),
    (game) async {
      game.spawnWave(1);
      await game.ready();
      ...
      final enemyBullets = game.children.whereType<BulletComponent>().where(
        (b) => b.isEnemy,
      );
  ...
  testWithGame<InvaderGame>(
    'should fire an enemy bullet once the shot interval has elapsed',
    () => InvaderGame(session: GameSession()),
    (game) async {
      game.spawnWave(1);
      await game.ready();
  ...
  testWithGame<InvaderGame>(
    'should move the boss horizontally on update during a boss wave',
    () => InvaderGame(session: GameSession()),
    (game) async {
      game.spawnWave(5);
      await game.ready();
  ...
  testWidgets(
    'should advance to the next wave after the delayed timer elapses when not stopped',
    (tester) async {
      final session = GameSession();
      final game = await initializeGame<InvaderGame>(
        () => InvaderGame(session: session),
      );
      game.spawnWave(1);
      game.update(0);
  ...
  testWidgets(
    'should not advance the wave once stop() has been called, guarding a stale timer',
    (tester) async {
      final session = GameSession();
      final game = await initializeGame<InvaderGame>(
        () => InvaderGame(session: session),
